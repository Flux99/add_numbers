...
def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        while top <= bot:
            ...
            if target > matrix[row][-1]:
                ...
                break # if we break here that means data is found before while loop ends, right ?

        if not (top <= bot): # and if data is found before while loop ends then why we're checking this condition ? . because it will fail , right ?
            return False
